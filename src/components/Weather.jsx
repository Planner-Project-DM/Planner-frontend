...
export default function Weather({activeTrip}) {
    ...
    async function handleCheckWeather() {
        ...
        setWeatherList(forecastResp.data.list)
        setWeatherData(mappedData);
        setIsLoading(false);
    }

    const hourly = useMemo(() => {
        const hourForecast = weatherList.filter((el) => el.dt_txt.includes(selectedDay))
        const mappedForecast = hourForecast.map((el) => ({
    ...
    }, [weatherList, selectedDay]);
    return (
        <div className={"flex h-full w-full justify-center items-center p-5 overflow-y-hidden"}>
            <div
                className={"flex flex-col h-full bg-bg-funds-card w-full justify-center items-center rounded-2xl " +
                    "shadow-md gap-5 overflow-y-scroll [&::-webkit-scrollbar]:hidden"}>
                <div className="mx-auto w-full flex justify-center items-center">
                    {isLoading && <p className="text-text-muted font-semibold text-center">Pobieranie danych...</p>}

                    {!isLoading && weatherData && (
                        <div className="rounded-3xl shadow-xl overflow-hidden border border-border-col w-11/12 h-11/12">
                            <div className="bg-gradient-to-br from-blue-500 to-blue-400 text-white p-5">
                                <div className={"flex justify-between"}>
                                    ...
                                    <div className={"flex gap-5 items-center justify-center"}>
                                        <FormInput shadow={"shadow-md"} maxh={"max-h-12"} type={"text"} value={weatherForm.city}
                                                   onChange={(e) => setWeatherForm({
                                                       ...weatherForm,
                                                       city: e.target.value
                                                   })}/>
                                        <button
                            ...
                            <div
                                className="bg-bg-funds-card p-6 flex justify-evenly items-center border-t border-border-col gap-3">
                                {hourly.map((hour, index) => (
                                    <div key={index}
                                        ...
                                        />
                                        <p className="text-sm font-bold text-text-main mt-1">{hour.temperature.current}°</p>
                                    </div>
                                ))}
                            </div>
                            <div
                                className="bg-bg-funds-card p-6 flex justify-between items-center border-t border-border-col">
                                {weatherData.forecast.map((day, index) => (
                                    <div key={index}
                                         onClick={() => setSelectedDay(day.date)}
                                         className="flex flex-col min-w-36 items-center bg-bg-card border border-border-col p-2 rounded-2xl shadow-sm cursor-pointer hover:bg-bg-main">
                                        <p className="text-xs font-bold text-text-muted mb-1">
                                            {day.date.slice(5)}
                                        </p>
                                        <img
                                            ...
                                            className="w-12 h-12 filter drop-shadow-[0_2px_4px_rgba(0,0,0,0.15)]"
                                        />
                                        <p className="text-sm font-bold text-text-main mt-1">Maks. {day.temperature.max}°</p>
                                        <p className="text-xs font-medium text-text-secondary">Min. {day.temperature.min}°</p>
                                    </div>
                                ))}
                            </div>
                        </div>
